...
TARGET_CYCLE_SEC = 5.0     # 실시간 관제 스캔 주기 (5.0초)
WAIT_RESPONSE_SEC = 0.20   # ESP32 응답 수신 대기 타임아웃 (200ms)
...
def send_plc_16bit_bytes(msb_byte: int, lsb_byte: int, expected_bytes: int = 2, wait_sec: float = WAIT_RESPONSE_SEC) -> bytes:
    """
    MSB 1바이트 + LSB 1바이트 전송 후 200ms 동안 수신 대기
    """
    global ser
    ...
    try:
        ...
        # 16비트 프레임 전송: [Byte 0: MSB] [Byte 1: LSB]
...
def connect_and_run():
    ...
    try:
        ...
        print(f"⚡ [{TARGET_CYCLE_SEC}초 관제 시작] 1~{TOTAL_RAILS}번 난간 실시간 센서 폴링 중...\n")
        ...
        while True:
            ...
            for rail_id in range(1, TOTAL_RAILS + 1):
                ...
                resp = send_plc_16bit_bytes(msb_byte=poll_msb, lsb_byte=0xFF, expected_bytes=5, wait_sec=WAIT_RESPONSE_SEC)

                if resp and len(resp) == 5 and resp[0] == rail_id:
                    l_mv = (resp[1] << 8) | resp[2]
                    r_mv = (resp[3] << 8) | resp[4]
                    left_watt = round((l_mv / 1000.0) * 1.2, 1)
                    right_watt = round((r_mv / 1000.0) * 1.1, 1)
                    battery_pct = round(max(0.0, min(100.0, ((l_mv / 1000.0) - 10.5) / (13.0 - 10.5) * 100.0)), 1)
                    is_error = False
                    success_count += 1
                    ...
                    is_error = True
                    left_watt, right_watt, battery_pct = 0.0, 0.0, 0.0

                is_emergency = (battery_pct < 10.0) and not is_error
            ...
            print(f"📡 [{TARGET_CYCLE_SEC}초 관제 완료] 전체 {TOTAL_RAILS}개 중 {success_count}개 정상 수신 (소요시간: {scan_elapsed_time:.2f}초)")
